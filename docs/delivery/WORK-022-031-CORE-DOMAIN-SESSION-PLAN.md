...
DEC-013 resolves the contracts that previously blocked readiness. The primary goal selects the protocol; secondary goals remain rationale-only customization with explicit Arabic messages. Reverse planning uses UTC `YYYY-MM-DD` calendar dates, inclusive today/deadline days, pages-per-minute from remaining pages and available minutes, and ceiling daily targets. Executable ordered step templates and default durations are the versioned `PROTOCOL_STEP_CATALOG` values. Version 1 fingerprints use normalized title, normalized author, and total pages; hash matches warn and require exact-metadata confirmation. The fourteen linked tasks remain `Inbox` until these contracts are reflected in their individual acceptance records.
...
SPEC-014 is In Review. The local implementation slice now includes real IndexedDB integration tests, migration write gating, explicit recovery persistence, preview/recall artifact saves, timer controls, Library resume/archive actions, and a runtime future-version read-only gate with an export action. `npm run check` passes 138 tests across 31 files; TypeScript, the Vite production build, and `git diff --check` pass. Packaged Chrome for Testing `152.0.7977.83` + ChromeDriver `152.0.7977.83` and Edge `152.0.4191.66` + EdgeDriver `152.0.4191.66` pass the persistence/session flow, true second-session restart/recovery, future-version migration-banner/export checks, and both record zero extension console errors and zero extension-originated network requests. Chrome restart keeps an ordinary target before session recreation; only toolbar action triggering is blocked by the WebDriver DevTools allowlist. The ten primary work items and four persistence enablers remain `Inbox` with DoR and DoD incomplete until per-task acceptance mappings and Notion status updates are recorded. `WORK-022` remains the first pure-domain candidate after DEC-013's decision matrix contract. Persistence-bearing work follows the explicit WORK-041 → WORK-044 gates above.
