...
SPEC-014 is Approved. The local implementation slice includes real IndexedDB integration tests, migration write gating and a runtime future-version read-only gate with an export action, explicit recovery persistence, preview/recall artifact saves, timer controls, Library resume/archive actions, and the guided flow. `npm run check` passes 138 tests across 31 files; TypeScript, the Vite production build, and `git diff --check` pass. Packaged Chrome for Testing `152.0.7977.83` + ChromeDriver `152.0.7977.83` and Edge `152.0.4191.66` + EdgeDriver `152.0.4191.66` pass the persistence/session flow, true second-session restart/recovery, future-version migration-banner/export checks, and both record zero extension console errors and zero extension-originated network requests. Chrome restart keeps an ordinary target before session recreation; only toolbar action triggering is blocked by the WebDriver DevTools allowlist. All ten primary tasks and four persistence enablers have per-task evidence, DoR/DoD complete, and `Done` status in Notion. DEC-013 records the approved rules, planning, step, and fingerprint contracts.
