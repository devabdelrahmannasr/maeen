...
The marketplace screenshot set uses the owner-provided PNG exports under [`docs/release/store-assets/`](docs/release/store-assets/). They contain synthetic Arabic RTL UI only, with no private user data or PDF content. Branding sources remain [maeen-logo.svg](public/branding/maeen-logo.svg) and [maeen-cover.svg](docs/release/maeen-cover.svg). The package remains unpublished by choice; marketplace publication is outside this internal-alpha release.

### Figma screen gallery

These are the supplied marketplace screens. `protocol-steps-420.png` is retained as a separate supplied file; it is byte-identical to `protocol-420.png` because both files were delivered as the same Figma export. The Arabic copy is product UI content, while this repository's documentation remains English for contributors.
...
| 6. Protocol steps | ![Protocol steps](docs/release/store-assets/protocol-steps-420.png) |
| 7. Focus session | ![Focus session](docs/release/store-assets/focus-600-dark.png) |
| 8. Recall and explain | ![Recall and explain](docs/release/store-assets/recall-420.png) |
| 9. Session summary | ![Session summary](docs/release/store-assets/summary-420.png) |
| 10. Book progress | ![Book progress](docs/release/store-assets/progress-600.png) |
