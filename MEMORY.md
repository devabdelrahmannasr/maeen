...
- `SPEC-014` is In Review and coordinates WORK-022 through WORK-031 plus WORK-041 through WORK-044. Persistence is now implementation scope through the explicit IndexedDB → migrations/read-only → autosave/integrity → interrupted-recovery chain. All fourteen items remain Inbox with DoR incomplete. Goal matrix/messages, reverse-planning semantics, executable protocol steps, and the metadata fingerprint contract still require approval before their owning work moves Ready. Do not substitute localStorage or fake-only repositories.
