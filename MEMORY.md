...
- Name: **Maeen — Reading Helper**. Repository/project name: **Maeen**.
- Promise: **Tell me why you are reading, and I will organize how you read.**
...
- Import validation rejects unsupported versions, duplicate/missing references, forbidden document/network fields, invalid timestamps, and oversized text before writes; commit uses one IndexedDB transaction with rollback and a local phase-only recovery marker.
...
- `SPEC-015` is Approved and its coordinated plan `docs/delivery/WORK-032-040-045-ARTIFACTS-METRICS-EXPORT-PLAN.md` is complete. WORK-032–036 post-focus artifacts/notes/gaps/applications/distractions, WORK-037–040 local metrics/Book Progress/resume/privacy, and WORK-045 JSON/Markdown export are implemented, browser-verified, and all ten Work Items are Done with DoR/DoD complete in Notion.
...
- Runtime migration handling is now browser-visible: a future IndexedDB version is detected without opening it for upgrade, domain writes are gated, and the app exposes a read-only warning plus `Export a backup`. The suite is 138 tests across 31 files; matching Chrome/Edge packaged runs pass the future-version banner/export-action check. Edge still provides the true second-session restart evidence; Chrome remains refresh-only under the ChromeDriver extension-target limitation. Console/network instrumentation and per-task Notion evidence are still required before closing SPEC-014 work items.
...
- SPEC-015 implementation and verification completed 2026-09-15. The suite passes 146 tests across 37 files plus TypeScript/Vite build; the packaged Chrome for Testing `152.0.7977.82` and Edge `153.0.4234.32` matrices with matching drivers cover post-focus artifacts, autosave/retry, distraction continuity, deterministic metrics, Book Progress/privacy, explicit JSON/Markdown export, restart/recovery, migration read-only/export, and zero extension console/network activity. The toolbar action remains blocked only by WebDriver's DevTools allowlist; direct registered Side Panel verification is the documented automation path. Import validation/preview/backup/commit was completed in SPEC-016 WORK-046–048.
- Final Edge rerun used the exact Microsoft Edge WebDriver `153.0.4234.32` matching the installed browser and exercised the migration export action; Chrome used matching ChromeDriver `152.0.7977.82`. Both final packaged runs passed with zero extension console errors and zero extension-originated network requests.
- SPEC-016 completion: WORK-046–056 are Done/DoR/DoD complete in Notion. The final suite passes 155 tests across 39 files with TypeScript/Vite; packaged Chrome 152.0.7977.82 and Edge 153.0.4234.32 matching-driver runs cover import preview/cancel/commit, rollback journal, restart/recovery, migration readonly/export, RTL, and zero extension console/network activity. Toolbar action triggering remains limited by the WebDriver DevTools allowlist.
- SPEC-016 is Approved and complete for WORK-046–WORK-056. The local plan `docs/delivery/WORK-046-056-IMPORT-QUALITY-RELEASE-PLAN.md` records implementation and evidence for import validation/preview, backup and rollback, export-import round trips, Arabic-first copy, RTL/themes, keyboard/focus/contrast, reduced motion/system states, unit/integration/E2E tests, and packaged Chrome/Edge release regression. All eleven tasks are Done with DoR/DoD complete.
- SPEC-017 / WORK-057–060 implementation is complete and closed in Notion: MIT/open-source docs, security/privacy contracts, deterministic store-draft assets, changelog/metadata, pinned reproducible release verification, CI, and the REL-002 internal-alpha evidence package. Chrome 152.0.7977.82 and Edge 153.0.4234.32 matching-driver runs passed with zero extension console/network errors; toolbar action remains a WebDriver allowlist limitation. Canonical submission URLs remain prerequisites to external publication.
- On 2026-09-16 the brand was refreshed to **Maeen — Reading Helper** across runtime/docs/local design exports. The alpha package was rebuilt and release verification still passed; remote Figma text-node edits require a later Figma MCP/browser session because the Starter quota/session is unavailable.
